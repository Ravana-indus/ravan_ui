(() => {
    ...
    function getFrappeThemePreference() {
        if (!window.localStorage) {
            return null;
        }
    ...
    function persistPreferences() {
        ...
        if (['light', 'dark'].includes(state.theme)) {
            localStorage.setItem(FRAPPE_THEME_KEY, state.theme);
        } else {
            localStorage.removeItem(FRAPPE_THEME_KEY);
        }
    }
    ...
    function observeThemeAttribute() {
        ...
        const observer = new MutationObserver(mutations => {
            mutations.forEach(mutation => {
                ...
                const nextTheme = rawTheme === 'dark' ? 'dark' : 'light';
